...
public class CadastroClientes {
   // Atributos
	...
	public Vector<Cliente> cosultarEmail (String email) throws Exception{
		Vector<Cliente> resposta = new Vector<Cliente>();
		for (Cliente obj : listaClientes) {
			if(obj.getEmail().toUpperCase().indexOf(email.toUpperCase()) != -1) {
				resposta.add(obj);
			}
		}
		if(resposta.isEmpty()) {
			throw new Exception("Não existe cliente com o email informado.");
			
		}else return resposta;
		
	}
	public Vector<Cliente> consultarTag (String tags) throws Exception{
		Vector<Cliente> resposta = new Vector<Cliente>();
		for(Cliente obj: listaClientes) {
			if(obj.getTags().toUpperCase().indexOf(tags.toUpperCase()) != -1) {
				resposta.add(obj);
			}
		}
		if(resposta.isEmpty()) {
			throw new Exception("Não existe cliente com a tag informada.");
		}else return resposta;
	}
